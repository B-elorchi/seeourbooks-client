...
export default function CostsByUserPage() {
  ...
  const { data, isLoading } = useAdminCostsByUser(days, PAGE_SIZE, offset)
  ...
  const chartData = rows.slice(0, 15).map(r => ({
    name: r.label.slice(0, 18) + (r.label.length > 18 ? '…' : ''),
    cost_usd: r.cost_usd,
  }))
  ...
  return (
    <PageShell>
      <PageHeader
        title="Costs by User"
        subtitle="All user costs with charts and breakdown tables"
        action={
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <KpiCard label="Users" value={rows.length} />
      ...
      {/* Table */}
      <Section title="Users table" action={
        <div className="flex items-center gap-2">
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          {isLoading ? (
            ...
            <p className="p-5 text-sm text-gray-400">No user cost data.</p>
          ) : (
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50 text-gray-500 uppercase tracking-wide">
                  <th className="text-left px-4 py-2.5">User</th>
                  <th className="text-left px-4 py-2.5 hidden sm:table-cell">Role</th>
                ...
                {rows.map(u => (
                  <tr key={u.user_id} className="hover:bg-gray-50">
                    <td className="px-4 py-2.5">
                      <span className="text-gray-800 truncate max-w-[260px] block">{u.label}</span>
                      <span className="text-[10px] text-gray-400 font-mono">ID: {u.user_id}</span>
                    </td>
                    <td className="px-4 py-2.5 text-gray-600 hidden sm:table-cell">{u.role || '—'}</td>
                    <td className="px-4 py-2.5 text-right text-gray-700">{u.total_jobs ?? 0}</td>
                    <td className="px-4 py-2.5 text-right text-gray-700">{u.total_calls ?? 0}</td>
                    <td className="px-4 py-2.5 text-right font-medium text-indigo-700">${u.cost_usd.toFixed(4)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
